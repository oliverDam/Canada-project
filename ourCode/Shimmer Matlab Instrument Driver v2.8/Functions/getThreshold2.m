function [threshold, output] = getThreshold2(dataInput, press, sens)
...
%  with size and threshold to adjust when movement is detected. Now with
%  foot pressure sensors as well.

moveMatrix = [];
moveMatrixCorrected = [];
meanPress = mean(mean(press));
...
for i=1:length(dataInput)
    if xthres >= dataInput(i,1) && ythres >= dataInput(i,2) && zthres ...
            >= dataInput(i,3) && mean(press(i,:)) >= meanPress*sens(3)
