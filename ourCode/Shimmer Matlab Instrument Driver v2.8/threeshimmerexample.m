...
if (shimmer1.connect && shimmer2.connect && shimmer3.connect)               % TRUE if both shimmer 1 and shimmer 2 connect
    ...
    if (shimmer1.start && shimmer2.start && shimmer3.start)                % TRUE if both shimmers start streaming
        ...
        while (elapsedTime < captureDuration)            
            ...
            if (firsttime1==true && isempty(uncalibratedData)~=1)
                firsttime1 = writeHeadersToFile(fileName1,signalName,signalFormat,signalUnit);
            else
                dlmwrite(fileName1, uncalibratedData, '-append', 'delimiter', '\t','precision',16);
            end
            ...
            % Part of saving - 5 lines:
            if (firsttime2==true && isempty(uncalibratedData)~=1)
                firsttime2 = writeHeadersToFile(fileName2,signalName,signalFormat,signalUnit);
            else
                dlmwrite(fileName2, uncalibratedData, '-append', 'delimiter', '\t','precision',16);
            end
